/*
 * Interrupt descriptor table and the central dispatcher.
 *
 * Fault isolation: if a CPU exception fires while an application is
 * running, the app is killed and control returns to the shell via
 * longjmp. Exceptions with no app running are kernel bugs -> panic.
 */
#include "kernel.h"
...
extern void idt_flush(u32 idtr);
...
static void idt_set(int n, u32 base)
{
...
#define SET(n) idt_set(n, (u32)isr##n);
...
#undef SET
#define SET(n, v) idt_set(v, (u32)irq##n);
...
void interrupt_dispatch(regs_t *r)
{
    ...
    if (current_process && current_process->running) {
        ...
        kprintf("\n[fault] %s (int %d, err %x) at eip=%p",
                name, r->int_no, r->err_code, r->eip);
        if (r->int_no == 14)
            kprintf(" addr=%p", read_cr2());
